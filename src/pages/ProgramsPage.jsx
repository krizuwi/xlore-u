  ...
  return (
    <section className="page-shell">
      <div className="container">
        ...
        {loading ? <LoadingState label="Loading programs..." /> : <div className="program-grid">{programs.map((program) => <article className="program-card" key={program.id}><div className="program-card-icon"><BookOpen /></div><span className="section-kicker">{program.category}</span><h2>{program.name}</h2><p>{program.description}</p><div className="program-meta"><span>{program.degreeLevel}</span><span>{program.schoolCount} schools</span></div><div className="career-list"><strong><BriefcaseBusiness size={15} /> Possible careers</strong><div>{(program.careerPaths ?? []).slice(0, 3).map((career) => <span className="tag" key={career}>{career}</span>)}</div></div><details><summary>Requirements and details</summary><p>{program.requirements}</p></details></article>)}</div>}
      </div>
    </section>
  );
}
