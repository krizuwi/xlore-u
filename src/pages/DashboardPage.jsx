import { ArrowRight, BookOpen, ClipboardCheck, Heart, RotateCcw, Scale, School, Sparkles } from "lucide-react";
    ...
    <section className="page-shell dashboard-page">
      <div className="container">
        ...
        {dashboard && <><div className="dashboard-stats"><Link to="/saved"><Heart /><span><small>Saved schools</small><strong>{dashboard.counts.savedSchools}</strong></span></Link><Link to="/saved"><BookOpen /><span><small>Saved programs</small><strong>{dashboard.counts.savedPrograms}</strong></span></Link><Link to="/assessment"><ClipboardCheck /><span><small>Assessments</small><strong>{dashboard.counts.assessments}</strong></span></Link></div>
          <div className="dashboard-grid"><section className="content-card"><div className="card-heading"><div><span className="section-kicker">Latest direction</span><h2>{dashboard.latestAssessment?.primaryDirection ?? "No assessment yet"}</h2></div><Sparkles /></div>{dashboard.latestAssessment ? <><p>Your latest profile is ready. Revisit its recommendations or retake the assessment as your interests develop.</p><div className="dashboard-assessment-actions"><Link className="text-btn inline-link" to={`/assessment/${dashboard.latestAssessment.assessmentId}/results`}>View results <ArrowRight size={14} /></Link><Link className="text-btn inline-link" to="/assessment">Retake assessment <RotateCcw size={14} /></Link></div></> : <><p>Complete the short profile assessment to unlock personalized program and school recommendations.</p><Link className="primary-btn compact" to="/assessment">Start now</Link></>}</section>
          ...
          <section className="content-card history-card"><div className="card-heading"><div><span className="section-kicker">Assessment history</span><h2>Your previous results</h2></div></div>{assessments.length ? <div className="history-list">{assessments.map((assessment) => <Link to={`/assessment/${assessment.id}/results`} key={assessment.id}><span className="history-icon"><ClipboardCheck /></span><span><strong>{assessment.primaryDirection}</strong><small>{new Date(assessment.completedAt).toLocaleDateString(undefined, { dateStyle: "medium" })}</small></span><span className="match-pill">View results</span></Link>)}</div> : <div className="empty-inline">No completed assessments yet.</div>}</section></>}
      </div>
    </section>
  );
}
